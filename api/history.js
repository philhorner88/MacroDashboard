export default async function handler(req, res) {
  ...
  const tickers = (req.query?.s || '').split(',').filter(Boolean)
  const from    = req.query?.from || ''
  const to      = req.query?.to   || new Date().toISOString().split('T')[0]

  if (!tickers.length || !from) return res.status(400).json({ error: 'tickers and from date required' })

  const fetchOne = async (ticker) => {
    try {
      const url  = `https://eodhd.com/api/eod/${ticker}?api_token=${apiKey}&fmt=json&from=${from}&to=${to}&period=d`
