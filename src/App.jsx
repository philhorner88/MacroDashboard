  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8', fontFamily: "'Inter', 'Segoe UI', sans-serif" }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          {loading ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#3182ce' }}>
              ...
              <span>Loading prices ({loaded}/{total})…</span>
            </div>
          ) : (
            <div style={{ fontSize: 12, color: '#a0aec0' }}>
              ✓ {loaded}/{total} prices loaded
              {lastFetch && <> · {lastFetch}</>}
            </div>
          )}
          <button
      ...
      <div style={{
        ...
        fontSize: 11, color: '#cbd5e1', borderTop: '1px solid #e2e8f0',
        background: '#fff', marginTop: 16,
      }}>
        Data via EODHD · Prices are end-of-day · Values from Sharesight snapshot 13 Mar 2026
      </div>
    </div>
  )
}
