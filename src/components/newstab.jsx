...
function SentimentBadge({ sentiment }) {
  ...
  return (
    <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 10, background: bg, color }}>
      {icon} {p}
    </span>
  )
}
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#4a5568', marginRight: 4 }}>Filter by:</div>
        <button onClick={() => setTicker('all')} style={{
          padding: '6px 14px', borderRadius: 20, fontSize: 12, fontWeight: 600,
          border: ticker === 'all' ? 'none' : '1px solid #e2e8f0',
          background: ticker === 'all' ? '#3182ce' : '#fff',
          color: ticker === 'all' ? '#fff' : '#4a5568', cursor: 'pointer',
        }}>
          Top 10 Holdings
        </button>
        {TOP_TICKERS.map(t => (
        ...
        <button onClick={load} disabled={loading} style={{
          ...
          border: '1px solid #e2e8f0', background: '#fff', color: '#3182ce', cursor: 'pointer',
        }}>
          ↺ Refresh
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 40, color: '#a0aec0', fontSize: 14 }}>No news found</div>
      ) : (
            ...
            >
              ...
              >
                <div style={{ display: 'flex', gap: 12, justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, fontSize: 14, color: '#2d3748', marginBottom: 6, lineHeight: 1.4 }}>
                      {item.title}
                    </div>
